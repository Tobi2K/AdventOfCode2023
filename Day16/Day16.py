...
def create_energy_map(maze, start_config=((1, 1), "R")):
    ...
    move_dict = {"U": {".": "U", "/": "R", "$": "L"}, "L": {".": "L", "/": "D", "$": "U"}, "D": {".": "D", "/": "L", "$": "R"}, "R": {".": "R", "/": "U", "$": "D"}}
    ...
    while len(positions) > 0:
        ...
        if cur_char == "-":
            ...
            # Else create two beams, one left one right
                ...
                positions.append((tuple([sum(x) for x in zip(cur_pos, move["L"])]), "L"))
                positions.append((tuple([sum(x) for x in zip(cur_pos, move["R"])]), "R"))
                continue
        elif cur_char == "|":
            # Do not split beam if we're moving with the pipe direction
            if direction in ["U", "D"]:
            ...
            # Else create two beams, one up one down
                positions.append((tuple([sum(x) for x in zip(cur_pos, move["U"])]), "U"))
                positions.append((tuple([sum(x) for x in zip(cur_pos, move["D"])]), "D"))
                continue
