    ...
    for line in inp:
        ...
        if not parse_parts:
            ...
            for rule in w_rules:
                ...
                parsed_rules.append({"cat": category, "op": operator, "value": value, "next": next_step})
            
            workflow_dict[w_name] = {"rules": parsed_rules, "last": w_last}
        else:
            # Match string and save according to structure above
            rates = re.search(r"\{x=(\d+),m=(\d+),a=(\d+),s=(\d+)\}", line)
            ratings.append({"x": int(rates.group(1)), "m": int(rates.group(2)), "a": int(rates.group(3)), "s": int(rates.group(4))})
...
def second_task(path):
    ...
    return calculate_possible_combinations(workflow_dict, "in", {"x": (1, 4000), "m": (1, 4000), "a": (1, 4000), "s": (1, 4000)})
...
def calculate_possible_combinations(workflow_dict, workflow_name, ranges):
    if workflow_name == "R":
        ...
        return math.prod((range_end - range_start + 1) for range_start, range_end in ranges.values())
    combinations = 0
    ...
    for rule in workflow["rules"]:
        ...
        if rule["op"] == ">":
            ...
            combinations += calculate_possible_combinations(workflow_dict, rule["next"], copy_range_upper)
        elif rule["op"] == "<":
            copy_range_lower = copy.deepcopy(ranges)
            copy_range_upper = copy.deepcopy(ranges)
            # Range that fulfills condition --> Continue with 'next' workflow
            ...
            # Add the combinations that we achieve if we 'branch off' from our current part
            combinations += calculate_possible_combinations(workflow_dict, rule["next"], copy_range_lower)
        else:
            raise Exception("Illegal operator received")
    
    # Add combinations for the last step in our workflow
    return combinations + calculate_possible_combinations(workflow_dict, workflow["last"], ranges)
